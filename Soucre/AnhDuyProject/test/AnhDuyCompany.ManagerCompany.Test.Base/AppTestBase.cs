...
using AnhDuyCompany.ManagerCompany.Test.Base.TestData;
...
namespace AnhDuyCompany.ManagerCompany.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="ManagerCompanyDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(ManagerCompanyDbContext context)
            {
        ...
        protected void UsingDbContext(Action<ManagerCompanyDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ManagerCompanyDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<ManagerCompanyDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<ManagerCompanyDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ManagerCompanyDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ManagerCompanyDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ManagerCompanyDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ManagerCompanyDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<ManagerCompanyDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<ManagerCompanyDbContext, Task<TResult>> func)
        {
            TResult result;
