...
namespace AnhDuyCompany.ManagerCompany.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : ManagerCompanyServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("AnhDuyCompany.ManagerCompany", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
